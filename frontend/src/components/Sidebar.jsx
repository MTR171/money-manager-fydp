...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Main Sidebar Aside */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-slate-900 border-r border-gray-100 dark:border-slate-800 transform transition-all duration-300 flex flex-col ${
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-gray-100 dark:border-slate-800">
          ...
          <div>
            <h1 className="font-bold text-gray-800 dark:text-slate-100 text-sm leading-tight tracking-tight">
              Money Manager
            </h1>
            <p className="text-[11px] text-gray-400 dark:text-slate-500 mt-0.5">AI Finance Tracker</p>
          </div>
        </div>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group relative ${
                  active
                    ...
                    : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800/70 hover:text-gray-800 dark:hover:text-slate-100'
                }`}
              >
        ...
        <div className="px-3 pb-4 pt-2 border-t border-gray-100 dark:border-slate-800 space-y-0.5">
          {/* Settings Nav Item */}
          {(() => {
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors"
          >
            <LogOut size={17} />
            <span>Logout</span>
          </button>
          ...
          {/* User Profile Card */}
          <div className="flex items-center gap-3 px-3 pt-2 mt-1 border-t border-gray-100 dark:border-slate-800">
          ...
          <div className="mt-2 pt-2 border-t border-gray-100 dark:border-slate-800 px-3 flex items-center justify-between text-xs text-gray-400 dark:text-slate-500 font-mono">
            <div className="flex items-center gap-1.5">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500" />
              </span>
              <span>v{typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : '1.2.7'}</span>
              <span className="text-gray-300 dark:text-slate-600">•</span>
              <span className="text-emerald-600 dark:text-emerald-400 font-medium font-sans">Stable</span>
            </div>
            {typeof __BUILD_DATE__ !== 'undefined' && (
              <span className="text-[10px] text-gray-400 dark:text-slate-500">{__BUILD_DATE__}</span>
            )}
          </div>
        </div>
      </aside>
